...
class SpiralGenerator(object):
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        """
        Boundary conditions for the segments of the generated spiral polynomial
        path. Variables:
            > kappa_lim is the curvature boundary (can be changed)
            > sf_0 is the path length
        """
        
        bounds = [[-kappa_lim, kappa_lim],
                  [-kappa_lim, kappa_lim],
                  [sf_0, None]]
    ...
    def thetat(self, a, b, c, d, s):
        
        thetas = [a*x + (b/2)*x**2 + (c/3)*x**3 + (d/4)*x**4 for x in s]
        return thetas
    
    def curvature(self, a, b, c, d, s):
        
        curv = [a + b*x**2 + c*x**3 + d*x**4 for x in s]
        return curv
    ...
    def sample_spiral(self, p):
        ...
        """
        Refer to Coursera Motion Planning for Autonomous Car about Conformal Lattice Planner
        """
        ...
        t_points = self.thetat(a, b, c, d, s_points)
        self._curvature = self.curvature(a, b, c, d, s_points)
