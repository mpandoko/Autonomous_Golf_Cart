// udah bagusssssssss tinggal rapihin aja
#include <ros/ros.h>
...
int main(int argc, char**argv) {
    ...
    while(ros::ok()) {
      ...
      // else if (init) {
        vx = (long double) v * cos(yaw);
        vy = (long double) v * sin(yaw);

        // dt = (current_time - last_time).toSec();
        delta_x = (long double) vx * dt;
        delta_y = (long double) vy * dt;

        x = (long double) x + delta_x;
        y = (long double) y + delta_y;
      // }
        //update transform
