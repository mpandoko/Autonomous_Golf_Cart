...
################################################################################################
var_imu_f = 0.10
var_imu_w = 0.25 #for part 3, I use var_imu_w = 1.00
var_gnss  = 0.01 #for part 3, I use var_gnss = 0.05
var_lidar = 1.00 # for part 2, I use var_lidar = 10
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    p_check = p_check + dx[0:3]
    v_check = v_check + dx[3:6]
    axis_angle = np.array([dx[6], dx[7], dx[8]])
    q_check = Quaternion(axis_angle =  dx[6:9]).quat_mult_left(q_check)

    # 3.4 Compute corrected covariance
    identity = np.eye(9)
    p_cov_check = (identity - K @ h_jac) @ p_cov_check
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p_est[k-1] + v_est[k-1] * delta_t + delta_t**2 / 2 * (C_ns @ imu_f.data[k-1] - g)
    ...
    p_cov_check = F_jac @ p_cov[k-1] @ F_jac.T + l_jac @ Q @ l_jac.T
    ...
    # 3. Check availability of GNSS and LIDAR measurements and update the states
    if lidar_i < lidar.t.shape[0] and lidar.t[lidar_i] == imu_f.t[k-1]:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov_check, lidar.data[lidar_i], p_check, v_check, q_check)
        lidar_i += 1

    if gnss_i < gnss.t.shape[0] and gnss.t[gnss_i] == imu_f.t[k-1]:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov_check, gnss.data[gnss_i], p_check, v_check, q_check)
        gnss_i += 1
