...
Mat X(4,1); // state variables
Vec u(2); // input variables: ax, ay, vyaw
Mat Q(4,4); // process noise covariance
...
Mat P(4,4); // state covariance
Mat sigma_points(4,N_sigma); // to store sigma points
long double dt; // delta time every 1 loop
Mat F(4,4); // motion model matrix
Mat u_mat(4,2); // input matrix in motion model
Vec weights(N_sigma);
Vec Pxy_tach(4); // cross covariance from predicted tachometer measurements
Vec Pxy_imu(4); // cross covariance from predicted IMU measurements
Mat Pxy_gnss(4,2); // cross covariance from predicted GNSS measurements
Vec K_tach(4); // Kalman gain for tachometer
Vec K_imu(4); // Kalman gain for IMU
Mat K_gnss(4,2); // Kalman gain for GNSS
...
void tachCallback(const geometry_msgs::Twist &tach_msg) {
    ...
    // v_new = vel;
    // u << ((v_new - v_old) / dt), imu.angular_velocity.z;
    ...
    for(i = 0; i < N_sigma; i++) {
      y_tach_estimate[i] = sigma_points.col(i)[2];
...
void imuCallback(const sensor_msgs::Imu &imu_msg) {
    ...
    for(i = 0; i < N_sigma; i++) {
      y_imu_estimate[i] = sigma_points.col(i)[3];
    }
...
int main(int argc, char**argv) {
  ...
  while(ros::ok()) {
    ...
    // if(!initialized && (imu.header.seq > 0)) {
      ...
      u_mat << cos(X(3,0)) * pow(dt, 2) / 2, 0,
              sin(X(3,0)) * pow(dt, 2) / 2, 0,
              dt, 0,
              0, dt;
      myfile << "u_mat =";
      for(i = 0; i < N; i++) {
      ...
      v_new = X(2);
      //
      if (v_new - v_old < 0.001) {
        u << 0.1, imu.angular_velocity.z;
      }
      else {
        u << (v_new - v_old) / dt, imu.angular_velocity.z;
      }
    ...
    X(3,0) = wrap_angle(X(3,0));
    yaw_odom = wrap_angle(X(3,0) + pi/2);
    ...
    odom.twist.twist.linear.x = X(2,0) * cos(X(3,0));
    odom.twist.twist.linear.y = X(2,0) * sin(X(3,0));
    odom.twist.twist.linear.z = 0.0;
    odom.twist.twist.angular.x = 0.0;
    odom.twist.twist.angular.y = 0.0;
    odom.twist.twist.angular.z = u[1];
