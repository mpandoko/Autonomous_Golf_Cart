...
def main():
    def callback_gps(msg_gps):
        ...
        state['x'] = msg_gps.pose.pose.position.x
        state['y'] = msg_gps.pose.pose.position.y
    ...
    rospy.Subscriber('/utm', Odometry, callback_gps)
    pub = rospy.Publisher('/get_waypoints', State_EKF_2D, queue_size=1)
